...
def naive_multi_task_model(market_classes,duke_classes,cuhk_classes):
    ...
    #market
    m_x = Dropout(0.5)(x)
    m_x = Dense(512, name='fc7_1', activation='relu')(m_x)
    m_x = Dropout(0.5)(m_x)
    ...
    #duke
    d_x = Dropout(0.5)(x)
    d_x = Dense(512, name='fc7_2', activation='relu')(d_x)
    d_x = Dropout(0.5)(d_x)
    ...
    #cuhk
    c_x = Dropout(0.5)(x)
    c_x = Dense(512, name='fc7_3', activation='relu')(c_x)
    c_x = Dropout(0.5)(m_x)
...
def run_model(is_single,is_naive,is_alternative,is_all_in_one,lr,loss_weights_1,loss_weights_2,loss_weights_3,dataset_name_1,dataset_name_2,dataset_name_3,batch_size,epoch,stored_model_path,log_path):
    ...
    all_in_one_model = alternate_multi_task_model(751,702,743)
    ...
    all_in_one_model.compile(optimizer=SGD(lr=lr, momentum=0.9), loss='categorical_crossentropy',metrics=['accuracy'])
    ...
    EPOCH_NB = epoch
    BATCH_NB_data = np.min([train_num_1,train_num_2,train_num_3])
    ...
    all_in_one_market_accuracy = 0
    all_in_one_duke_accuracy = 0
    all_in_one_cuhk_accuracy = 0
    ...
    for epoch_nb in xrange(EPOCH_NB):
        ...
        Losses_32=[]
        Losses_33=[]
        for batch_nb in tqdm(xrange(BATCH_NB_data), total=BATCH_NB_data):
            ...
            Image_data_ = np.concatenate([Image_data_1['input'],Image_data_2['input'],Image_data_3['input']],axis=0)
            Labels_1_ = np.concatenate([Labels_1['label'],np.zeros([batch_size,702]),np.zeros([batch_size,743])],axis=1)
            Labels_2_ = np.concatenate([np.zeros([batch_size,751]),Labels_2['label'],np.zeros([batch_size,743])],axis=1)
            Labels_3_ = np.concatenate([np.zeros([batch_size,751]),np.zeros([batch_size,702]),Labels_3['label']],axis=1)
            Labels_data_  = np.concatenate([Labels_1_,Labels_2_,Labels_3_],axis=0)

            index =  [i for i in range(0,3*batch_size)]
            shuffle(index)

            Image_data__ = np.zeros([batch_size*3,224,224,3])
            Labels_data__ = np.zeros([batch_size*3,751+702+743])
            j=0
            for i in index:
                Image_data__[j] = Image_data_[i]
                Labels_data__[j] = Labels_data_[i]
                j=j+1

            Image_data = {'input':Image_data__}
            #print Labels_data__.shape
            #print Labels_data__[:,:751].shape
            #Labels  = {'label_1':Labels_data__[:][:751],'label_2':Labels_data__[:][751:1453],'label_3':Labels_data__[:][1453:]}

            if is_all_in_one:
                losses = all_in_one_model.train_on_batch( Image_data['input'],
                                          [Labels_data__[:,:751],Labels_data__[:,751:1453], Labels_data__[:,1453:]])
                write_log(callback,all_in_one_names,losses,batch_nb)
        ...
        if is_all_in_one:
            if (np.array(Losses).mean(axis=0)[4]>all_in_one_market_accuracy) & (np.array(Losses).mean(axis=0)[5]>all_in_one_duke_accuracy) & (np.array(Losses).mean(axis=0)[1]>all_in_one_cuhk_accuracy):
                all_in_one_market_accuracy = np.array(Losses).mean(axis=0)[4]
                all_in_one_duke_accuracy = np.array(Losses).mean(axis=0)[5]
                all_in_one_cuhk_accuracy = np.array(Losses).mean(axis=0)[6]
                all_in_one_model.save(stored_model_path+str(epoch_nb)+'_'+str(all_in_one_market_accuracy)+'_'+str(all_in_one_duke_accuracy)+'_'+str(all_in_one_cuhk_accuracy)+'__'+'weights.h5')
            print 'Done for Epoch %d.'% epoch_nb
            print  'alternate_market_train_result:',np.array(Losses).mean(axis=0)
...
if __name__ == '__main__':
    ...
    all_in_one_names = ['all_in_one_loss','market_loss','duke_loss','cuhk_loss']
    
    log_path,stored_model_path=path_file(isCenterloss,model_names,task_name)
    run_model(is_single,is_naive,is_alternative,is_all_in_one,lr,loss_weights_1,loss_weights_2,loss_weights_3,dataset_name_1,dataset_name_2,dataset_name_3,batch_size,epoch,stored_model_path,log_path)
